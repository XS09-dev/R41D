...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Shit', 0.2], //From 20% to 39%
		['Bad', 0.3], //From 40% to 49%
		['Bruh', 0.4], //From 50% to 59%
		['Meh', 0.6], //From 60% to 68%
		['Nice', 0.6], //69%
		['Good', 0.7], //From 70% to 79%
		['Great', 0.8], //From 80% to 89%
		['Sick!', 1.5], //From 90% to 99%
		['Perfect!!', 2] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var tankBuildings:FlxSprite;
	var tankRuins:FlxSprite;

	var tankStep:Ts;

	var tower:FlxSprite;
	var tankmanRun:FlxTypedGroup<TankmanKilled>;	
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'tank': //Week 7 - Stress, Guns, Ugh 
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.anyJustPressed(debugKeysCharacter) && !endingSong && !inCutscene && !isStoryMode) {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'picospeakershoot1':
				gf.playAnim('shoot1', true);
				gf.specialAnim = true;

			case 'picospeakershoot2':
				gf.playAnim('shoot2', true);
				gf.specialAnim = true;		
					
			case 'picospeakershoot3':
				gf.playAnim('shoot3', true);
				gf.specialAnim = true;
					
			case 'picospeakershoot4':
					...
					for (epicNote in sortedNotesList)
					{
						for (doubleNote in pressNotes) {
							if (Math.abs(doubleNote.strumTime - epicNote.strumTime) < 1) {
								doubleNote.kill();
								modchartObjects.remove('note${doubleNote.ID}');
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				case 'PreetyGoodNote': //Tankman shit
						if(dad.animation.getByName('prettyGoodAnim') != null) {
		...
		if(SONG.song.toLowerCase() == 'stress')
			{
				...
				for (i in 0...tankStep.left.length)
				{
					if (curStep == tankStep.left[i]){
						var tankmanRunner:TankmanKilled = new TankmanKilled();
